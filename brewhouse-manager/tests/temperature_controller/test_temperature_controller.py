...
def test_compare_temperature():
    sensors = [MockSensor(readings=[50.0, 59.1, 60.1, 61.0])]
    mock_heater = MockHeater()

    temp_controller = TemperatureController(
        sensors, mode=ControllerMode.HEATER, heater=mock_heater)
...
def test_tick():
    heater_sensor = [MockSensor(readings=[50.0, 59.9, 60.0, 60.1])]
    mock_heater = MockHeater()
    ...
    cooler_sensor = [MockSensor(readings=[70.0, 60.1, 59.9, 59.0])]
